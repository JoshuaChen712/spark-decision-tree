    ...
    def build_tree(self):
        ...
        while not node_queue.empty():
            ...
            if node.node_type != TreeNodeType.ROOT:
                selected_columns = [col for col in self.data.columns if col not in node.excluded_features]
                node_data = self.data.filter(node.condition).select(selected_columns)
            else:
                node_data = self.data
            node_data.cache()
    ...
    def info_gain(self, 
        ...
        if self.treeType == "ID3" or self.treeType == "C4.5":
            ...
            if self.treeType == "C4.5":
                tmp_df = joined_df.withColumn('ratio', joined_df['sum(count)']/total_num)
...
class DFDecisionTreeModel():
    def __init__(self,
                 ...
                 labelCol: Optional[str] = None,
                 treeType: Optional[str] = "ID3"):
        self.decision_tree = DecisionTree(maxDepth, maxBins, numclasses, featureCols, featureTypes, labelCol, treeType)
